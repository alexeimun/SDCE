    ...
    <div class="login-box-body " id="contenedor" style="opacity: .95;">
        ...
        <?= form_open('app/ValidarCredenciales', ['method' => 'post']) ?>
            <div class="form-group has-feedback">
                <input autofocus type="text" class="form-control" placeholder="Ingrese su correo universitario" name="CORREO"/>
...
<script>
    $('button:submit').click(function () {
        ...
        $.ajax({
            url: '<?=site_url('seguimiento/validaringreso')?>', type: 'post', data: $('form').serialize()+'&ID_PRACTICANTE=<?=$_GET['_id']?>&CONSECUTIVO=<?=$_GET['_cvo']?>',
            success: function (data) {

                if (data == 'ok') {
                    location.href = '<?=site_url('informe/asesoriapracticas')?>';
                }
